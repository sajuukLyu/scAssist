...
plotFeature <- function(
  ...
  if(cbar == "normal") {
    ...
    warning("Sorry, this colorbar style not supported, set to 'normal'.")
    x <- plotData$feat
    b <- min(x) + diff(range(x))*seq(0, 1, len = 5)
    plotColorBar <- scale_color_gradientn(
      colors = col, values = disp,
  ...
  # split
  if(split) {
    splitOrder <- levels(plotData$split)
  }
  ...
  if(order == "value") {
    ...
    warning("Sorry, this order style not supported, set to 'value'.")
    plotDataOrdered <- plotData[order(feat)]
  }
